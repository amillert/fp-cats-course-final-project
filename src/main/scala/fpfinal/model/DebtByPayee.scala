...
object DebtByPayee {
  ...
  /** TODO: Implement a monoid instance.
    ...
    */
  implicit def monoidDebtByPayee(
      implicit
      monoidMap: Monoid[Map[Person, Money]]
    ): Monoid[DebtByPayee] = ???

  implicit def showDebtByPayee(
      implicit
    ...
    Show.show { d =>
      d.allPayees()
        .foldMap(payee => s"- ${payee.show}: ${d.debtForPayee(payee).getOrElse(Money.zero).show}\n")
    }
}
